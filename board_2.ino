...
const char* ssid = "ESP32_AP_2";
const char* password = "12345678";
AsyncWebServer server(80);

String subStationNames[46] = {
  "CTRE", "MDVE", "TUG", "WIR" ,"THL" ,"KPD", "VI", "GYM" ,
   "VLT" ,"PCKM" ,"VGM" ,"KDY", "AVN", "SHU", "MA", "MCN" ,
   "SVUR" ,"LTI" ,"KVN" ,"MEH" ,"MPI", "AB" ,"VN" , // 0-22
   "MLMR" ,"ACK", "TZT", "OLA", "TMV" ,"MTL", "PEI", "WN", "MYP", //23-31
   "TCCV", "REN", "SIV", "NGV", "TRL", "KZD", "NYY",  
  "KAN", "VACAPA", "MCG", "PYD", "PASA", "TVC", "RTY" //32-44
};
...
void handlePage1(AsyncWebServerRequest *request) {
  ...
  // Station 1: Arakonam
  page += "<div class='section station'><h2>Station 1: Arakonam</h2>";
  ...
  // Sub-Stations for Kanyakumari
  page += "<div class='section substations'><h2>Sub-Stations (Arakonam)</h2><div class='controls'>";
  for (int i = 0; i < 23; i++) {
    page += getSubstationButton(1, i, subStationNames[i], subStationStatus[i]);
  }
  page += "</div></div>";

  // Station 2: Jolarpettai
  page += "<div class='section station'><h2>Station 2: Jolarpettai</h2>";
  page += "<div class='controls'><h3>Track Controls</h3>";
...
void handlePage2(AsyncWebServerRequest *request) {
  ...
  // Station 1: Chengalpattu
  page += "<div class='section station'><h2>Station 1: Chengalpattu</h2>";
  page += "<div class='controls'><h3>Track Controls</h3>";
  ...
  // Sub-Stations for Chengalpattu
  page += "<div class='section substations'><h2>Sub-Stations (Chengalpattu)</h2><div class='controls'>";
  for (int i = 23; i < 32; i++) {
    page += getSubstationButton(2, i, subStationNames[i], subStationStatus[i]);
  }
  page += "</div></div>";

  // Station 2:Villupuram
  page += "<div class='section station'><h2>Station 2: Villupuram</h2>";
  page += "<div class='controls'><h3>Track Controls</h3>";
...
void handlePage3(AsyncWebServerRequest *request) {
  ...
  // Sub-Stations
  page += "<div class='section substations'><h2>Sub-Stations (Kanyakumari) </h2><div class='controls'>";
  for (int i = 32; i < 45; i++) {
    page += getSubstationButton(3, i, subStationNames[i], subStationStatus[i]);
  }
  page += "</div></div>";

  // Station 2: Thiruvananthapuram
  page += "<div class='section station'><h2>Station 2:  Thiruvananthapuram</h2>";
  page += "<div class='controls'><h3>Track Controls</h3>";
...
String getPageFooter() {
  ...
  footer += "        document.getElementById('status_' + pin).innerText = data.status;";
  footer += "      }";
  footer += "    });";
  footer += "}";
...
void handleToggle(AsyncWebServerRequest *request) {
  if (request->hasParam("page") && request->hasParam("pin") && request->hasParam("type")) {
    ...
    String stationName;
    switch (pageIndex) {
      case 0: stationName = "Arakonam"; break;
      case 1: stationName = "Jolarpettai"; break;
      case 2: stationName = "Chengalpattu"; break;
      case 3: stationName = "Villupuram"; break;
    ...
    if (type == "track" || type == "front" || type == "back") {
      ...
      if ((pageIndex < 3 && subStationIndex < 47) || (pageIndex >= 3 && subStationIndex < 47)) {
...
void handleSpeedChange(AsyncWebServerRequest *request) {
  if (request->hasParam("track") && request->hasParam("speed") && request->hasParam("trackNumber")) {
    ...
    String trackName;
    switch (track) {
        case 0: stationName = "Arakonam"; trackName = (trackNumber == 0) ? "UpTrack 1" : "DownTrack 1"; break;
      case 1: stationName = "Jolarpettai"; trackName = (trackNumber == 0) ? "UpTrack 1" : "DownTrack 1"; break;
      case 2: stationName = "Chengalpattu"; trackName = (trackNumber == 0) ? "UpTrack 1" : "DownTrack 1"; break;
      case 3: stationName = "Villupuram"; trackName = (trackNumber == 0) ? "UpTrack 1" : "DownTrack 1"; break;
